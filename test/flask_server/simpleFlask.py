...
class TestApp(unittest.TestCase):
    ...
    def test_postSimpleApi(self):
        with patch('subprocess.getoutput') as mock_getoutput:
            mock_getoutput.return_value = 'user test created!'
            response = self.app.post('/simpleApi', json={'name': 'test'})
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.data.decode(), 'user test created!')
    ...
    def test_getAuthenApi_authenticated(self):
        with patch('flask.request.cookies.get') as mock_get:
