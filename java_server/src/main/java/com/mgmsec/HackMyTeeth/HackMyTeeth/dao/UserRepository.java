...
public class UserRepository {
    ...
    public String findByUsername(String username, String password) {
        ...
        List<User> result = jdbcTemplate.query("SELECT * FROM user WHERE username=? and password=?",
                (rs, rowNum) -> new User(rs.getLong("userID"), rs.getString("firstName"), rs.getString("lastName"), rs.getString("email"), rs.getString("username"), rs.getString("password"), rs.getString("role"),rs.getString("salt")), username, password);
        String s1 = "0";
        String s2 = "1";
        if (result.isEmpty()) {
            return "invalid";
